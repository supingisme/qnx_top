...
uint64_t  min_mem_avail = 1000000;   // units - MB
uint64_t  max_mem_avail;
uint64_t  tot_mem_avail;
    ...
    if (dir == NULL) {
        ...
        while ((dirent = readdir(dir)) != NULL) {
            if (isdigit(dirent->d_name[0])) {
                ...
                pe_p = do_process(node, atoi(dirent->d_name), 0);
                if (pe_p != NULL) {
    ...
    )
{
    ...
    for (pe_p = now_p->process_list_p;
		...
		if (tpid > 0 && tpid != te_p->status.pid)
			continue;
...
static int get_section_size(const char * sec_name_pstr, uint64_t* sec_tot_mem_pu64)
{
	struct asinfo_entry *l_asinfo_pst;
	char *l_names_pstr=NULL;
	int i = 0;
	int l_num_asinfos =_syspage_ptr->asinfo.entry_size / (sizeof(struct asinfo_entry));

	l_asinfo_pst = SYSPAGE_ENTRY(asinfo);

	*sec_tot_mem_pu64 = 0;
	for(i=0 ; i < l_num_asinfos;i++)
	{
		l_names_pstr = SYSPAGE_ENTRY(strings)->data;
		if (strcmp(&l_names_pstr[l_asinfo_pst->name],sec_name_pstr) == 0)
		{

			*sec_tot_mem_pu64 += l_asinfo_pst->end -l_asinfo_pst->start + 1;
		}
		l_asinfo_pst++;
	}
	return 0;
...
static int get_free_mem(uint64_t* free_mem_pu64) 
{
	struct stat l_stat_buf_st;
	if (stat("/proc", &l_stat_buf_st) == -1)
	{
		printf("stat on /proc failed errno[%d] %s \n",errno,strerror(errno));
		return -1;
	}
	*free_mem_pu64 = l_stat_buf_st.st_size;
	return 0;
}
...
void print_memory (char * suffix)
{
	...
	printf("Memory: %luMB total, %luMB avail, page size %uKB%s\n",
		total, free, private->pagesize/1024, suffix);

        min_mem_avail = min(free, min_mem_avail);
        max_mem_avail = max(total, max_mem_avail);
        tot_mem_avail+= free;
}
...
/* Get a process thread neme from pid and tid */

void get_thread_name (pid_t pid, pthread_t tid, char *threadname)
{
    int  ret;
...
void top (char * node, int iterations)
{
    ...
    while (!quit) {
        ...
        printf( "      PID   TID PRI STATE    HH:MM:SS    CPU   LASTCPU  COMMAND              THREAD%s\n",
                ce_sequence);
        ...
        for (i = 0; i < active; i++) {
            ...
            get_thread_name(te_p->status.pid, te_p->status.tid, threadname);

            printf("%9d %5d %3d %4s %s %3lld.%02lld%% %8d  %-20s %-20s %s\n",
                   te_p->status.pid,
                   te_p->status.tid,
...
int main (int ac, char * av[])
{
    ...
    while ((c = getopt(ac, av, "p:i:t:dn:")) != EOF) {
        switch (c) {
        case 'p':
	    {
        ...
        default:
            ...
            printf(" -t <pid>   show one process or thread only\n");
            exit(0);
        break;
        }
    }
